...
void Auto_Leave(double current_time, double start_time){
  double release_time=1000,back_time=1000+release_time; //三个阶段时间
  double push_rotation=(RightPush.rotation(vex::rotationUnits::deg)+LeftPush.rotation(vex::rotationUnits::deg))/2;
  ...
  LeftUpIntake.stop(vex::brakeType::coast);
  RightUpIntake.stop(vex::brakeType::coast);
  if(current_time-start_time<=release_time){
    move(0,0);
    Push(0);
  }
  else if(current_time-start_time<=back_time){
    Push(-13000);
    Intake(-6000);
  }
  else {
